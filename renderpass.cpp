...
void RenderPass::create(Context* context_, VkFormat colorFormat, VkFormat depthFormat, VkSampleCountFlagBits sampleCount) {
	context = context_;
	...
	if (vkCreateRenderPass(context->device, &renderPassInfo, nullptr, &handle) != VK_SUCCESS) {
		throw std::runtime_error("failed to create render pass!");
	}
}

void RenderPass::create(Context* context_, VkFormat depthFormat) {
	context = context_;
...
void RenderPass::create(Context* context_, VkFormat colorFormat, VkFormat depthFormat) {
	context = context_;

	std::array<VkAttachmentDescription,3> colorAttachment{};
	for (VkAttachmentDescription& e : colorAttachment) {
		e.format = colorFormat;
		e.samples = VK_SAMPLE_COUNT_1_BIT;
		e.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
		e.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
	...
	std::array<VkAttachmentDescription, 4> attachments = { colorAttachment[0], colorAttachment[1],colorAttachment[2], depthAttachment };
...
void RenderPass::createFramebuffers(SwapChain& swapchain, Image& color, Image& depth, uint32_t width, uint32_t height) {
...
void RenderPass::createFramebuffers(Image& depth, uint32_t width, uint32_t height) {
	uint32_t num = 1;
	framebuffers.resize(num);
	for (uint32_t i = 0; i < num; ++i) {
...
VkFramebuffer RenderPass::getFramebuffer(uint32_t index) { return framebuffers[index]; }
